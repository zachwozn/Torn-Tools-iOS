import { D as formatDate, H as elementBuilder, J as sortTable, U as findAllElements, W as getSearchParameters, _ as stakeouts, d as loadDatabase, i as sendMessage, k as formatTime, n as initializeInternalPage, r as loadConfirmationPopup, rt as ttStorage, s as attackHistory, t as getPageTheme, tt as PHTrash, u as initializeDatabase, y as storageListeners } from "./pages-DbN4bb4V.js";
import "./browser-DV2XfOQj.js";
/* empty css               */
import { t as Sortable } from "./sortable.esm-DaFN78Tm.js";
//#region extension/entrypoints/targets/targets.ts
var initiatedPages = {};
(async () => {
...
async function setupAttackHistory() {
	...
	const historyList = _attackHistory.querySelector("#attacksList");
	fillHistory();
	sortTable(historyList, 3, "desc");
	_attackHistory.querySelector("#percentageHistory").addEventListener("click", (event) => {
		_attackHistory.querySelector("#attacksList").classList[event.target.checked ? "add" : "remove"]("switched");
	});
	_attackHistory.querySelector("#resetHistory").addEventListener("click", () => {
		loadConfirmationPopup({
			...
			for (const row of findAllElements("tr.row", _attackHistory)) row.remove();
		}).catch(() => {});
	});
	function fillHistory() {
		for (const id in attackHistory.history) addHistoryRow(id, attackHistory.history[id]);
	}
	function addHistoryRow(id, data) {
		const row = elementBuilder({
			type: "tr",
			class: "row"
		});
		row.appendChild(elementBuilder({
			type: "td",
			class: "id",
		...
		if (data.lastAttackCode) row.appendChild(elementBuilder({
			type: "td",
			class: "last-attack",
		...
		else row.appendChild(elementBuilder({
			type: "td",
			class: "last-attack",
			text: lastAttackText,
			attributes: { value: data.lastAttack }
		...
		for (const type of [
			...
			const element = elementBuilder({
				type: "td",
				class: `data switchable ${type}`,
				attributes: { "sort-type": "css-dataset" }
			});
		...
		row.appendChild(elementBuilder({
			type: "td",
			class: "data assist",
			text: data.assist.toString(),
			attributes: { value: data.assist }
		}));
		row.appendChild(elementBuilder({
			type: "td",
			class: "data defend",
			text: data.defend.toString(),
			attributes: { value: data.defend }
		}));
		for (const type of [
			"lose",
			"stalemate",
		...
		if (data.respect_base.length) {
			...
			row.appendChild(elementBuilder({
				type: "td",
				class: "data respect",
				text: respect.toString(),
				attributes: { value: respect }
			}));
		...
		if (data.latestFairFightModifier) row.appendChild(elementBuilder({
			type: "td",
...
async function setupStakeouts() {
	...
	const stakeoutList = _stakeouts.querySelector("#stakeoutList");
	fillStakeouts();
	...
	_stakeouts.querySelector("#resetStakeouts").addEventListener("click", () => {
		loadConfirmationPopup({
			...
			for (const row of findAllElements("#stakeoutList tr.row")) row.remove();
		}).catch(() => {});
	});
	document.querySelector("#addStakeout").addEventListener("click", async () => {
		const id = document.querySelector("#stakeoutId").value;
		if (!id) return;
		if (document.querySelector(`#stakeout_${id}`)) sendMessage("This id already has a stakeout.", false);
		else addStakeout(parseInt(id));
		document.querySelector("#stakeoutId").value = "";
	});
	...
	function addStakeout(id, data = null, showStatus = true) {
		const row = elementBuilder({
			type: "tr",
			class: "row",
			id: `stakeout_${id}`,
		...
		alerts.push(elementBuilder({
			...
			children: [
				elementBuilder({
					type: "label",
					attributes: { for: `life-${id}` },
					text: "life drops below "
				}),
				elementBuilder({
					type: "input",
					id: `life-${id}`,
					class: "life short-input",
					attributes: {
						type: "number",
						min: 1,
						max: 100
			...
			children: [
				elementBuilder({
					type: "label",
					attributes: { for: `offline-${id}` },
					text: "offline for over "
				}),
				elementBuilder({
					type: "input",
					id: `offline-${id}`,
					class: "offline short-input",
					attributes: {
						type: "number",
						min: 1
					}
		...
		if (data?.alerts) for (const key in data.alerts) {
			if (!data.alerts[key]) continue;
			const element = alertsWrap.querySelector(`.${key}`);
			if (!element) continue;
	...
	function updateStakeouts() {
		findAllElements("tr:not(.header)", stakeoutList).filter((row) => !(parseInt(row.dataset.id) in stakeouts)).filter((row) => !row.classList.contains("new")).forEach((row) => row.remove());
		for (const id of stakeouts.order) {
			...
			if (Object.keys(stakeout.info).length) {
				...
				else row.querySelector(".name").appendChild(elementBuilder({
					type: "a",
			...
			alerts.querySelector(".okay").checked = stakeout.alerts.okay;
			alerts.querySelector(".hospital").checked = stakeout.alerts.hospital;
			alerts.querySelector(".landing").checked = stakeout.alerts.landing;
			alerts.querySelector(".online").checked = stakeout.alerts.online;
			alerts.querySelector(".life").value = stakeout.alerts.life.toString() || "";
			alerts.querySelector(".offline").value = stakeout.alerts.offline.toString() || "";
			alerts.querySelector(".revivable").checked = stakeout.alerts.revivable;
		}
	}
	async function saveStakeouts() {
		...
		for (const row of findAllElements("tr.row", stakeoutList)) {
			...
			newStakeouts[id] = {
				...
				alerts: {
					okay: alertsSection.querySelector(".okay").checked,
					hospital: alertsSection.querySelector(".hospital").checked,
					landing: alertsSection.querySelector(".landing").checked,
					online: alertsSection.querySelector(".online").checked,
					life: parseInt(alertsSection.querySelector(".life").value) || false,
					offline: parseInt(alertsSection.querySelector(".offline").value) || false,
					revivable: alertsSection.querySelector(".revivable").checked
				},
...
//# sourceMappingURL=targets-DVoVNSZd.js.map