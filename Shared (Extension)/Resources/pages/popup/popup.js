...
async function showPage(name) {
    const activePage = document.querySelector(`#${name}`);
    activePage.classList.add("active");

    document.querySelectorAll("body > main.subpage.active, #pages li.active").forEach(active => active.classList.remove("active"));

    const navItem = document.querySelector(`#pages li[to="${name}"]`);
    ...
    apiKeyButton.addEventListener("click", () => {
        ...
        checkAPIPermission(key)
            .then((granted) => {
                changeAPIKey(key)
                    .then(async () => {
                        pagesElement.classList.remove("tt-hidden");
                        ...
                        if (granted) {
                            ...
                            }, 10 * 1000); // Converted to milliseconds
                        }
                    })
