...
async function setupPreferences(requireCleanup) {
    ...
    async function saveSettings() {
        ...
        // Storing the settings using ttStorage
        try {
            const newStorage = { settings };
            sendMessage(newStorage);
            ...
            sendMessage("Failed to save settings. Please check the console for more details.", false);
        }
...
async function setupAPIInfo() {
    ...
    document.find("#update-torndata").addEventListener("click", () => {
        safari.extension.dispatchMessage("forceUpdate", { update: "torndata" });
    });

    document.find("#update-stocks").addEventListener("click", () => {
        safari.extension.dispatchMessage("forceUpdate", { update: "stocks" });
    });

    document.find("#update-factiondata").addEventListener("click", () => {
        safari.extension.dispatchMessage("forceUpdate", { update: "factiondata" });
    });

    // Listener to handle responses from the background script
    safari.self.addEventListener("message", (event) => {
        if (event.name === "updateResult") {
            console.log("Manually fetched " + event.message.update + ".", event.message);
            sendMessage(event.message.success ? "Fetched " + event.message.update + " successfully." : "Failed to fetch " + event.message.update + ".", event.message.success);
        }
    });
