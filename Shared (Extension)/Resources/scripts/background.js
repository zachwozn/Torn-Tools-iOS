...
async function setupExtension() {
    ...
    // Check for updates or perform version-specific migrations
    await checkUpdate();
...
//    setupAlarms();
    ...
    // Perform initial clearing and updates
    clearUsage();
    clearCache();
    timedUpdates();

    // Update UI components or perform initial setups
    await showIconBars();

    // Example of setting a listener for settings changes
    // This might be done via observing changes to localStorage in a real-world scenario
    // Or using the messaging system of Safari App Extensions to communicate changes
    settingsListeners.push(async () => {
        await showIconBars();
    });
}
...
async function updateUserdata() {
    ...
    console.log("Fetched Torn Data - ", userdata)
...
async function showIconBars() {
	if (!settings.apiUsage.user.bars || !hasAPIData() || !settings || !settings.pages.icon.global) {
		chrome.action.setIcon({ path: chrome.runtime.getURL("resource/images/icon_128.png") });
	} else {
		let barCount = 0;
...
function getNotificationSound(type) {
	return new Promise((resolve) => {
		switch (type) {
			...
			case "4":
			case "5":
				return resolve(`resource/audio/notification${type}.wav`);
			case "custom":
...
function createNotification(title, options) {
    if (!("Notification" in window)) {
        ...
        notification.onclick = function () {
            ...
            // Implement click response, e.g., opening a tab:
